      ...
      {cartProductsData?.map((item) => {
        const formatted =
          typeof item.productTotalPrice === 'number'
            ? Math.round(item.productTotalPrice * 100) / 100 % 1 === 0
              ? Math.round(item.productTotalPrice * 100) / 100
              : (Math.round(item.productTotalPrice * 100) / 100).toFixed(2)
            : '-';
        return (
          <div key={item.cartItemID} className={styles.card}>
            <div className={styles.imageWrapper}>
              <img src={`${BASE_URL}${item.product.image1}`} alt={item.product.productName} className={styles.productImage} />
            </div>
            <div className={styles.infoWrapper}>
              <div className={styles.headerRow}>
                <h3>{item.product.productName}</h3>
                <button className={styles.deleteBtn} onClick={() => onDelete(item.cartItemID)}>
                  <Image src="/cardDeleteIcon.svg" alt="delete" width={20} height={20} />
                </button>
              </div>
